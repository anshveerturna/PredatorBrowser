...
6. Workflow cache keyed by instruction + starting URL + environment, storing canonical `ActionContract` payloads for deterministic replay.
...
- cached action verification failure during replay
- selector no longer present in current structured state
- URL/elements state-hash mismatch (page structure drift)
